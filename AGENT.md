...
CMS ligero, open source (MIT), basado en Astro, pensado para desplegarse
...
theming complejo, sin control de scraping por IA).
...
| Atomic Elements (editor visual) | `packages/atomic-elements/` | ✅ Funcional, sin undo/redo ni anidamiento visual en columnas |
| Dashboard + Skin System | `packages/dashboard/` | ✅ Funcional |
| Centro de Permisos | `packages/permissions/` | 🟡 UI funcional, persistencia solo en memoria |
| Trust Layer (Web Bot Auth) | `packages/trust-layer/` | 🟡 Resuelve directorio de claves, NO valida firma criptográfica todavía (TODO explícito) |
| Sandboxing de plugins | `packages/plugin-sandbox/` | 🟡 4 adaptadores con contrato completo, NINGUNO ejecuta código real todavía (TODO explícito en cada uno) |
| Comercio como plugin sandboxeado | `packages/commerce-plugin/` | 🟡 Solo `manifest.json` de referencia, lógica real sigue en `src/commerce/` |
| MCP server | `packages/mcp-server/` | ❌ Stub vacío (`version: 0.0.0`), sin lógica |
...
| Autenticación / roles de usuario | No existe todavía | ❌ Pendiente — prioridad alta en `ROADMAP.md` |
...
- El Trust Layer no verifica firmas criptográficamente todavía — no lo presentes en ninguna documentación como "protección activa contra scraping".
- El Centro de Permisos y el ledger de trazabilidad viven en memoria — no son una fuente de auditoría confiable en producción todavía.
