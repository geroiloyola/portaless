...
//
// Se ejecuta en CI via .github/workflows/sandbox-e2e.yml (Node nativo con
// --test, sin dependencias de test runner externas).
...
test("hello-plugin ejecuta codigo real dentro del isolate y respeta la allowlist de red", async (t) => {
  ...
  assert.equal(result.success, true, `La ejecucion deberia tener exito. Error: ${result.error}`);
  assert.equal(result.provider, "node-isolated-vm");
  assert.equal(result.deniedCapabilityAttempts.length, 0, "No deberia haber intentos denegados con el codigo original de hello-plugin");
  assert.ok(result.durationMs >= 0);
});

test("el sandbox bloquea y registra un intento de red hacia un host no autorizado", async (t) => {
  let installedVersion: string;
  try {
    installedVersion = loadIsolatedVmVersion();
  } catch {
  ...
  assert.ok(result.deniedCapabilityAttempts.includes("network:fetch"), "El intento hacia evil.example.com debe quedar registrado como denegado");
});

test("el sandbox rechaza capacidades no concedidas aunque el manifiesto las declare", async (t) => {
  let installedVersion: string;
  try {
    installedVersion = loadIsolatedVmVersion();
  } catch {
  ...
  assert.equal(result.success, true);
  assert.ok(result.deniedCapabilityAttempts.includes("network:fetch"), "Sin concesion explicita del Centro de Permisos, network:fetch debe quedar denegado");
});
