...
- [ ] Integrar el Editor Visual de Atomic Elements al panel de administracion de produccion – HALLAZGO esta sesion: `public/editor/index.html` (canvas + paleta + drag-and-drop) NO esta enlazado desde `src/pages/admin/index.astro` (que solo linkea a Centro de Permisos y Site Trust Score) ni desde ninguna otra pagina `.astro` real. Es una demo standalone (mismo patron que `public/permissions/index.html`, documentado como tal en CHANGELOG.md), con su propio `elementRegistry` duplicado y reducido (6 elementos: Hero/Heading/Paragraph/Image/Button/Spacer) -- desactualizado respecto a los 12 elementos reales de `packages/atomic-elements/src/elements/registry.ts` (incluye ProductGrid, LinkList, SocialIcons, ProfileHeader, StoreBlock). La brecha real no es que "ProductGrid muestre un placeholder" -- es que el editor visual completo todavia no tiene una pagina de produccion (ej. `src/pages/admin/editor.astro`) que herede el layout del dashboard y la validacion de sesion nativa de Astro. **Trabajo ya adelantado, sin perdida**: `functions/admin/api/product-preview.js` (endpoint BFF, securizado con `context.data.user`, reutiliza `elementRegistry.ProductGrid.renderHTMLAsync()` real) ya esta listo para servir HTML real al editor de produccion en cuanto exista.
