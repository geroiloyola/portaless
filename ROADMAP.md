...
- [~] Ecosistema de plugins abierto+cerrado con trustScore comunitario -- v0.0.9.9, avanzado parcialmente; v0.0.9.10 conecto el registro al Centro de Permisos; v0.0.9.11 agrego persistencia real. Diseño inspirado en Shopify App Store / WooCommerce.org / Android Play Store: el catalogo de plugins es abierto por defecto (cualquiera puede registrar uno, `sourceType: "open"` o `"closed"`), y la comunidad regula la confianza via un `trustScore` publico (promedio de votos 1-5) en vez de que Portaless certifique nada. Se agrego `packages/plugin-sandbox/src/registry/plugin-registry.ts` (interfaz `PluginRegistryStore` + `InMemoryPluginRegistryStore` de referencia) y las tablas `plugin_registry`/`plugin_trust_votes` en `schema.sql`. **v0.0.9.10**: se agrego `packages/plugin-sandbox/src/registry/store-factory.ts` (`createPluginRegistryStore(env)`, seedeado con `hello-plugin`/`commerce-plugin`) y `functions/admin/permissions/index.js` ya deriva su catalogo de subjects de este registro real, no de una lista hardcodeada. **v0.0.9.11**: se agregaron `D1PluginRegistryStore` y `SqlitePluginRegistryStore` (mismo patron que `packages/permissions/src/stores/`), usando las tablas `plugin_registry`/`plugin_trust_votes` ya definidas en `schema.sql`; `store-factory.ts` ahora elige `env.DB` (D1) -> `env.PORTALESS_SQLITE_PATH` (SQLite self-hosted) -> memoria, igual que `createPermissionStore`. El registro ya no se resetea en cada despliegue si hay D1 o SQLite configurado -- puntos (1) y (2) de la lista original, cerrados. **Pendiente para cerrar esta linea del todo**: UI en el Centro de Permisos para mostrar el trustScore antes de conceder capacidades. Ver la nueva seccion "Ecosistema de plugins" mas abajo para la vision completa de largo plazo (compatibilidad con AT Protocol como posible AppView).
...
9. Agregar UI de trustScore en el Centro de Permisos (`D1PluginRegistryStore`/`SqlitePluginRegistryStore` ya implementados en v0.0.9.11, mismo patron que `permissions`; `functions/admin/permissions/index.js` conectado desde v0.0.9.10 -- ver `ROADMAP.md` seccion "Ecosistema de plugins").
