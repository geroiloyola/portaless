...
- [~] SiteTrustScore – capa de datos y endpoints HTTP para `authorized_agents`/`escrow_report` COMPLETADOS esta sesion, falta solo la UI. `AuthorizedAgentsStore` y `AuthorizedEscrowProvidersStore` (`packages/trust-layer/src/site-trust/`) ahora exponen `list()`/`grant()`/`revoke()` en las 3 implementaciones (D1, SQLite, InMemory) -- antes SOLO tenian `isAuthorized()`, sin ningun metodo de escritura, confirmado archivo por archivo. `grant()` de escrow providers genera la API key server-side (`crypto.randomUUID()`) y la devuelve en texto plano UNA sola vez -- solo el hash SHA-256 se persiste, sin ningun camino para recuperarla despues. `revoke()` en ambos stores hace soft-delete (`active = 0`), preservando el historial. Endpoints nuevos: `functions/admin/api/authorized-agents.js` y `functions/admin/api/authorized-escrow-providers.js` (GET/POST/PATCH, mismo guard `context.data.user`/`canWrite(role)` que `functions/admin/permissions/index.js`; PATCH en vez de DELETE para la revocacion, semantica de soft-delete). **Pendiente para cerrar esta linea por completo**: paginas `.astro` de UI (formularios de alta + tablas de listado/revocacion) enlazadas desde `src/pages/admin/index.astro`, mismo patron que `permissions.astro`. Ver desglose completo en la seccion nueva “Portaless Public” mas abajo.
