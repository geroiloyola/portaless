...
- [ ] SiteTrustScore – UI de alta para `authorized_agents` y `escrow_report` requiere trabajo previo mas profundo de lo documentado. HALLAZGO esta sesion, confirmado archivo por archivo (7 archivos totales que tocan estas 2 tablas, sin excepciones encontradas): la interfaz `AuthorizedAgentsStore` (`packages/trust-layer/src/site-trust/authorized-agents.ts`) SOLO expone `isAuthorized(agentKeyId): Promise<boolean>` -- ni la interfaz ni `D1AuthorizedAgentsStore` ni `SqliteAuthorizedAgentsStore` tienen metodos de escritura (`grant`/`revoke`/`list`). Mismo patron confirmado en `authorized-escrow-providers.ts`. El unico camino de alta hoy son `scripts/onboard-agent.mjs` y `scripts/onboard-escrow-provider.mjs` -- ambos son CLIs que escriben directo via `node:sqlite`, **exclusivamente self-hosted, sin cobertura para D1/Cloudflare Pages en absoluto** (no solo sin UI, sin ningun camino de alta si el despliegue usa D1). Antes de poder construir la UI de alta hace falta: (1) ampliar ambas interfaces con metodos de escritura reales; (2) implementarlos en las 4 clases concretas (D1 y SQLite, para agents y escrow providers); (3) crear los endpoints HTTP nuevos bajo `functions/admin/` con el mismo guard `context.data.user`/`canWrite(role)` que ya usa `functions/admin/permissions/index.js`; (4) recien despues, las paginas `.astro` de UI. Ver desglose completo en la seccion nueva “Portaless Public” mas abajo.
