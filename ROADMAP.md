...
- [~] Ecosistema de plugins abierto+cerrado con trustScore comunitario -- v0.0.9.9, avanzado parcialmente. Diseño inspirado en Shopify App Store / WooCommerce.org / Android Play Store: el catalogo de plugins es abierto por defecto (cualquiera puede registrar uno, `sourceType: "open"` o `"closed"`), y la comunidad regula la confianza via un `trustScore` publico (promedio de votos 1-5) en vez de que Portaless certifique nada. Se agrego `packages/plugin-sandbox/src/registry/plugin-registry.ts` (interfaz `PluginRegistryStore` + `InMemoryPluginRegistryStore` de referencia) y las tablas `plugin_registry`/`plugin_trust_votes` en `schema.sql`. **Pendiente para cerrar esta linea**: (1) implementar `D1PluginRegistryStore`/`SqlitePluginRegistryStore` (solo existe la version en memoria hoy); (2) conectar `functions/admin/permissions/index.js` a este registro real en vez del catalogo hardcodeado (`hello-plugin`, `commerce-plugin`) que sigue usando hoy; (3) UI en el Centro de Permisos para mostrar el trustScore antes de conceder capacidades. Ver la nueva seccion "Ecosistema de plugins" mas abajo para la vision completa de largo plazo (compatibilidad con AT Protocol como posible AppView).
...
9. Implementar `D1PluginRegistryStore` y `SqlitePluginRegistryStore` (v0.0.9.9 solo agrego la interfaz + InMemoryPluginRegistryStore); conectar `functions/admin/permissions/index.js` al registro real en vez del catalogo hardcodeado; agregar UI de trustScore en el Centro de Permisos.
