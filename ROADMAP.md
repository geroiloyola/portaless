...
> **Nota de mantenimiento (refactor v0.0.9.26, post PR #40):** este archivo fue reordenado para que cada entrada viva en una sola categoria segun su estado real verificado contra PRs mergeados y runs de CI. No se elimino ningun texto: las entradas duplicadas o desactualizadas se movieron a su seccion correcta y se marcaron con `[Refactor v0.0.9.26]` explicando el motivo. Ver "Registro del refactor" al final.
...
### Base del proyecto, autenticacion y licencias
...
### Persistencia, instalacion y datos
...
### Plugin sandbox, capacidades y Capability Bridge
...
- [x] SiteTrustScore – las 4 fuentes (self, agent, community, escrow_report) tienen endpoint HTTP conectado – esta sesion (rama `agentic`, commits `9c39ce1a0`, `f13cf3ee`, `d8e6ffd2`, `4154a4ebe`, `a6c356da5`). Diseño: `packages/trust-layer/src/site-trust/site-trust-score.ts` (dominio separado de `PluginRegistryStore`, que califica plugins, no sitios completos); 5 tablas nuevas en `schema.sql` (`site_trust_subjects` + una por fuente). Verificacion de identidad via `web-bot-auth.ts` (paquete oficial de Cloudflare, cache de JWKS via KV, TTL 6h) + `authorized_agents`/`authorized_escrow_providers` como allowlist de AUTORIZACION (identidad valida no es lo mismo que permiso – distincion de status code 401 vs 403). Endpoints: `functions/trust/[siteId]/agent-verification.js` y `.../escrow-report.js` (API key hasheada SHA-256, entrega manual fuera de banda, sin autoservicio). **Limitacion que persiste**: `agent` y `escrow_report` no tienen UI de escritura ni datos reales todavia – ambas allowlists estan vacias por defecto, alta manual sin flujo automatico. Ver docs/architecture/site-trust-score.md. **[Refactor v0.0.9.26]** La "limitacion que persiste" sobre UI de escritura ya NO aplica: la UI de alta/revocacion existe (ver la entrada "ciclo completo de `authorized_agents`/`escrow_report`" en esta subseccion). Lo unico vigente: allowlists vacias por defecto, sin datos de produccion.
- [x] Crypto-agilidad post-cuantica para `authorized_agents` (esquema de base de datos) – RESUELTO, verificado visualmente esta sesion via busqueda de codigo directa sobre `schema.sql` en `main`: el archivo maestro (usado para D1) SI tiene la columna `key_algorithm TEXT NOT NULL DEFAULT 'ed25519'` en la definicion real de la tabla `authorized_agents`, con nota inline v0.0.9.24 confirmando que `packages/trust-layer/src/site-trust/` no tiene un schema.sql de paquete individual propio (el maestro se edito directo, sin riesgo de desincronizacion entre paquete y maestro). La limitacion que este roadmap documentaba en sesiones anteriores ("schema.sql no se actualizo, D1 insertaria filas sin key_algorithm") era informacion desactualizada, no un riesgo real de infraestructura -- confirmado que nunca hubo asimetria real entre el schema.sql que usa SQLite y el que usa D1. Contexto: Ed25519 (usado hoy por Web Bot Auth, RFC 9421) es criptografia de curva eliptica clasica, vulnerable al algoritmo de Shor; NIST ya finalizo el reemplazo estandarizado (FIPS 204, ML-DSA, 13 de agosto de 2024, mismo estatus legal que AES/SHA-2). **Lo unico pendiente de esta linea, ahora exclusivamente de codigo TypeScript, no de base de datos**: la columna solo deja el esquema listo para no requerir una migracion de datos futura -- `web-bot-auth.ts` sigue verificando unicamente Ed25519, sin implementacion real de verificacion ML-DSA todavia. Ver docs/architecture/site-trust-score.md, seccion "Crypto-agilidad". **[Refactor v0.0.9.26]** Pendiente vigente movido como tarea propia a "Funcionalidades Internas en Desarrollo" (verificacion ML-DSA real en `web-bot-auth.ts`).
- [x] SiteTrustScore – ciclo completo de `authorized_agents`/`escrow_report` CERRADO end-to-end esta sesion (rama `agentic`, commits `5bd660b1`, `bcba3f56`, `50ad8f3b`, `aa5a2994`, `c4effcef`, `6506df49`, `7f7208d6`). Capa de datos: `AuthorizedAgentsStore` y `AuthorizedEscrowProvidersStore` (`packages/trust-layer/src/site-trust/`) exponen `list()`/`grant()`/`revoke()` en las 3 implementaciones (D1, SQLite, InMemory) -- antes SOLO tenian `isAuthorized()`, sin ningun metodo de escritura, confirmado archivo por archivo. `grant()` de escrow providers genera la API key server-side (`crypto.randomUUID()`) y la devuelve en texto plano UNA sola vez -- solo el hash SHA-256 se persiste, sin ningun camino para recuperarla despues. `revoke()` en ambos stores hace soft-delete (`active = 0`), preservando el historial. Endpoints: `functions/admin/api/authorized-agents.js` y `functions/admin/api/authorized-escrow-providers.js` (GET/POST/PATCH, mismo guard `context.data.user`/`canWrite(role)` que `functions/admin/permissions/index.js`; PATCH en vez de DELETE para la revocacion, semantica de soft-delete). UI: `src/pages/admin/authorized-agents.astro` y `.../authorized-escrow-providers.astro` (tablas de listado/revocacion + formulario de alta, mismo patron de estados carga/error que `permissions.astro`; el panel de escrow providers incluye un componente de "copia esta API key ahora, no volveras a verla" para el unico momento en que la clave existe en texto plano). Navegacion: ambas paginas enlazadas como tarjetas nuevas desde `src/pages/admin/index.astro`, mismo patron `ai-nav-card` que ya usaban Centro de Permisos y Site Trust Score. Un admin autenticado ya puede dar de alta o revocar agentes y proveedores de escrow sin tocar la terminal ni el CLI.
- [x] Protocol APW resolver real – CERRADO esta sesion (rama `agentic`, commits `e08f9c2`, `ffd2b43`, `945968a`, `405085f`, `b3ccb3a`, `43eb97e`). `packages/apw-resolver/src/dns-txt/dns-txt.ts` implementa `resolveApwTxtRecord()` via DNS-over-HTTPS (Cloudflare 1.1.1.1, `application/dns-json`) en vez del modulo nativo `dns` de Node -- decision deliberada: el runtime edge (Cloudflare Pages Functions, mismo runtime que ya usa `packages/trust-layer/src/webbotauth/`) no tiene sockets UDP/TCP nativos, y DoH corre igual en local que en cualquier plataforma edge sin comportamiento asimetrico. `packages/apw-resolver/src/manifest.ts` define el payload JSON compacto (`siteId`, `trustUrl` -> `/trust/:siteId`, `contentKinds`) con `buildApwManifest()`/`parseApwManifest()` sin lanzar excepciones para contenido invalido. `packages/apw-resolver/src/index.ts` expone `resolveApwManifest()` como punto de entrada unico (DNS + parseo combinados). `packages/apw-resolver/src/dnslink/cli.mjs` genera el valor exacto del TXT record listo para copiar/pegar, sin editar DNS de forma automatica (sin API estandarizada comun entre proveedores) y validando el limite practico de 512 bytes. Tests reales en `tests/unit/apw-dns-txt.test.ts` y `tests/unit/apw-manifest.test.ts`: mocks solo en el limite de red real (`fetch`), mismo estandar que `webbotauth-verify.test.ts`, mas un test de integracion contra el DoH publico real de Cloudflare (`cloudflare.com`, sin mocks) gateado por `PORTALESS_ENABLE_NETWORK_TESTS=1` para no romper CI sin red.
- [x] Identidad `did:apw` del sitio (generacion + persistencia cifrada) – CERRADO esta sesion (rama `agentic`, commits `0c5d117d`, `7472f093`). `packages/apw-resolver/src/did-apw/generate.ts` implementa `generateApwDid()` -- primera funcion del repo que GENERA un par de claves Ed25519 nuevo (antes solo existia verificacion de firmas ajenas en `webbotauth/verify.ts`). `document.ts` construye un documento DID W3C-compatible (`did:apw:tudominio.com`), mismo shape que `did:web` para maxima compatibilidad con resolvers de DID genericos. `site-identity-store.ts` persiste `publicKeyJwk` en claro y `privateKeyJwk` CIFRADO con AES-GCM (`PORTALESS_SITE_IDENTITY_ENCRYPTION_KEY`, env var, nunca en `schema.sql`) en la tabla nueva `site_identity`, con 3 implementaciones (D1/SQLite/InMemory) siguiendo el mismo patron que `AuthorizedEscrowProvidersStore`. `functions/admin/api/site-identity.js` expone `GET`/`POST` con el mismo guard `canWrite(role)` de siempre; el `POST` devuelve `privateKeyJwk` en texto plano UNA sola vez y responde 409 si el sitio ya tiene identidad (sin flujo de rotacion todavia). `manifest-with-identity.ts` conecta el `didDocument` al payload del TXT record de Protocol APW sin modificar `manifest.ts`. Tests reales sin mocks de criptografia en `apw-did.test.ts` y `apw-site-identity-store.test.ts`. **Pendiente real, no bloqueante**: `PORTALESS_SITE_IDENTITY_ENCRYPTION_KEY` es hoy un secreto de infraestructura fijo, no una contraseña maestra derivada por usuario -- mejora de seguridad valida para instalaciones multi-tenant, pero no urgente en el escenario single-tenant autohospedado actual (la variable de entorno ya protege la clave Ed25519 en reposo si alguien descarga el archivo SQLite). **[Refactor v0.0.9.26]** Pendientes vigentes movidos como tareas propias a "Funcionalidades Internas en Desarrollo": derivacion de la clave de cifrado y flujo de rotacion de identidad.
...
- [x] Confirmar los nombres exactos de los campos title/description en packages/atomic-elements/src/types.ts (PageLayout) – CERRADO, ver linea movida a “Funcionalidades en Produccion” arriba. **[Refactor v0.0.9.26]** Movida desde "Funcionalidades Internas en Desarrollo", donde figuraba como `[ ]` aunque el propio texto decia CERRADO (duplicado de "Campos title/description de PageLayout").
...
- [ ] Paso 1 del Wizard: endpoint que traduzca el prompt de intencion a `create_page` del MCP server (hoy solo guarda en `sessionStorage`). **[Refactor v0.0.9.26]** Tarea extraida de "Onboarding de un click" (Produccion) y de la seccion "Onboarding de un click: Wizard + Checklist diferido".
- [ ] Paso 2 del Wizard: flujo OAuth real con GitHub Pages / Cloudflare Pages (hoy UI marcada "Proximamente"). **[Refactor v0.0.9.26]** Misma procedencia que la tarea anterior.
...
- [ ] `ProductGrid` dentro del canvas del Editor Visual sigue mostrando un placeholder visual, no el HTML real de Medusa/Mercur -- a diferencia del `ProductGrid` del sitio publicado (`renderHTMLAsync()`, ya conectado desde v0.0.6). El editor (`editor-app.ts`, metodo `renderEditableNode`) hoy llama a `renderHTML()` (sincrono) para todos los elementos incluyendo `ProductGrid`, no a `renderHTMLAsync()`. `functions/admin/api/product-preview.js` ya expone el HTML real via BFF -- falta conectar ese endpoint desde el editor para que el placeholder se reemplace por una vista previa real dentro del canvas.
...
|SiteTrustScore – 4 fuentes (self, agent, community, escrow_report)|Las 4 fuentes tienen endpoint HTTP conectado. `agent` y `escrow_report` ya tienen ciclo completo de administracion (capa de datos + endpoints + UI `.astro` + navegacion desde el dashboard) -- un admin puede dar de alta/revocar sin depender de curl/fetch manual. Pendiente real: ambas allowlists siguen vacias por defecto (sin datos de produccion todavia) y no hay flujo de onboarding automatico para un segundo proveedor de escrow|
|Protocol APW (descubrimiento via DNS TXT `_apw.tudominio.com`)    |Implementado via DNS-over-HTTPS (Cloudflare 1.1.1.1). Ver commits `e08f9c2`..`43eb97e`. Pendiente: `did:web`/`did:apw` y consumo desde Portaless Index. |                                     | **[Refactor v0.0.9.26]** `did:apw` YA implementado (ver "Identidad did:apw" en Produccion). Sigue pendiente `did:web` y el consumo desde Portaless Index.

**Tareas – Protocol APW (CERRADO esta sesion, ver "Funcionalidades en Produccion"):**

- [x] Implementar `packages/apw-resolver/src/dns-txt/`: lectura real via DNS-over-HTTPS.
- [x] Definir el formato del payload (`siteId`, `trustUrl`, `contentKinds`) en `manifest.ts`.
- [x] Escribir el lado de publicacion (`dnslink/cli.mjs`).
- [x] Tests de resolucion reales, incluyendo integracion contra DoH real (gateada por `PORTALESS_ENABLE_NETWORK_TESTS=1`).

**Pendiente real, no cubierto por esta sesion**: `did:web`/`did:apw` (`src/did-apw/`) y el consumo de este resolver desde Portaless Index (ver tareas mas abajo, ninguna iniciada todavia). **[Refactor v0.0.9.26]** `did:apw` ya esta cerrado; lo vigente es `did:web` y Portaless Index.

**Tareas – SiteTrustScore (completar las 2 fuentes sin datos reales):**

- [x] UI de alta para `authorized_agents` – CERRADO esta sesion, ver linea movida a “Funcionalidades en Produccion”. `src/pages/admin/authorized-agents.astro` permite listar/dar de alta/revocar sin tocar la terminal.
- [ ] Flujo de onboarding de proveedores de `escrow_report` mas alla de la entrega manual de API key fuera de banda (decision deliberada como Fase 1, por ser la señal mas objetiva/ground truth) – la UI de alta ya existe (`src/pages/admin/authorized-escrow-providers.astro`), pero sin un segundo proveedor real registrado, el score no puede probarse con mas de una fuente objetiva.
...
## Onboarding de un click: Wizard + Checklist diferido

Seccion nueva (esta sesion). Diseño explicito, sin implementar todavia — ver "Funcionalidades Internas en Desarrollo" para el estado real. Resuelve la tension identificada en esta misma sesion: bloquear al usuario no tecnico con propagacion de DNS (que tarda minutos u horas) antes de dejarlo ver su sitio publicado mataria la conversion del publico objetivo de "Publico objetivo y casos de uso ideales" mas arriba. La solucion: separar en dos fases, nunca mezclarlas. **[Refactor v0.0.9.26]** Estado actualizado: ya NO esta "sin implementar" en su totalidad — la estructura del Wizard y el Paso 4 existen (ver "Tareas ... ESTADO REAL" abajo). Pendientes: Paso 1, Paso 2 y checklist diferido.

**Principio rector**: todo lo que depende de un tercero externo (propagacion DNS, verificacion de infraestructura ajena) pertenece al Dashboard como checklist diferido, nunca al Wizard bloqueante. El Wizard solo recolecta lo que es instantaneo y local.

### Fase 1 — Wizard bloqueante (paralelo a la configuracion inicial de un smartphone moderno)

| Paso generico de un smartphone | Paso Portaless | Por que es bloqueante |
|---|---|---|
| Idioma y region | Intencion del sitio: nombre, dominio, y un prompt libre ("¿De que trata este sitio?") que el agente MCP usa para pre-generar la primera pagina con los `ElementType` correctos | Sin esto no hay sitio que crear |
| Conectarse a una red Wi-Fi | Autorizacion OAuth con un proveedor de despliegue (GitHub Pages o Cloudflare Pages), para que el agente pueda desplegar | Sin infraestructura autorizada no hay donde publicar |
| Configuracion biometrica (el hardware de seguridad del dispositivo genera claves internamente) | Generacion invisible del par de claves Ed25519 (Web Bot Auth) en segundo plano, persistidas en D1/SQLite — el usuario solo ve una pantalla de carga ("Asegurando tu identidad soberana"), sin necesitar entender criptografia de curva eliptica | Es la base de identidad de todo el ecosistema (SiteTrustScore, Protocol APW, firma de imagenes futura) — debe existir desde el primer momento, no puede ser opcional ni diferida |
| Codigo de respaldo / copia de seguridad en la nube | Pantalla de exportacion de la clave privada en texto plano, una sola vez, con instruccion explicita de guardarla fuera de Portaless — mismo patron ya implementado en el onboarding de escrow providers (`authorized-escrow-providers.astro`, "copia esta API key ahora, no la volveras a ver") | Unico momento de friccion dura justificada: si el usuario pierde su base de datos sin haber guardado esta clave, pierde la identidad que respalda su `SiteTrustScore` y cualquier firma futura de Portaless Cloud Images |
| Credenciales de admin | Usuario/contraseña o proveedor OAuth para el panel de administracion | Requisito minimo para poder entrar al dashboard despues |

**Nota tecnica deliberada**: la "clave privada exportada" NO es una frase semilla derivada de un estandar de billeteras cripto — Ed25519 no deriva claves de una frase mnemonica salvo que se implemente esa capa explicitamente, y `web-bot-auth.ts` no lo hace hoy. Envolver la exportacion en un checksum de ese tipo es una mejora futura posible, no un bloqueante de la primera version de este wizard.

### Fase 2 — Checklist diferido dentro del dashboard (paralelo a "configurar mas tarde en los ajustes del sistema")
...
### Tareas para implementar esta seccion **[Refactor v0.0.9.26]** Encabezado historico sin contenido propio; el contenido vigente esta en el encabezado siguiente.

### Tareas para implementar esta seccion — ESTADO REAL esta sesion
...
1. Implementar `packages/apw-resolver/src/dns-txt/` (Protocol APW real) – ver desglose completo en la seccion “Portaless Public: descubrimiento y confianza entre sitios”. **[Refactor v0.0.9.26]** HECHO: ver "Protocol APW resolver real" en Produccion.
...
## Registro del refactor (v0.0.9.26)

- Produccion agrupada en subsecciones tematicas; ninguna entrada eliminada.
- Movidas de Internas a Produccion: "Ecosistema de plugins... CERRADO" (`[~]`→`[x]`), "Endpoint HTTP interno del CapabilityHostBridge" (`[~]`→`[x]`, marcada HISTORICA), "Confirmar title/description... CERRADO" (`[ ]`→`[x]`).
- Notas de correccion agregadas a entradas cerradas con informacion desactualizada: MCP server, SiteTrustScore (4 fuentes), Onboarding de un click, Editor Visual, Crypto-agilidad, did:apw, Capability Bridge, CI/CD.
- Tareas nuevas en Internas extraidas de pendientes que solo existian dentro de texto de entradas cerradas: `PORTALESS_CAPABILITY_BRIDGE_URL` en `.env.example`, ML-DSA, derivacion/rotacion de identidad, Paso 1 y Paso 2 del Wizard, checklist diferido, `GET /admin/pages`.
- Externas: integracion edge anotada con su bloqueo interno (`issueCapabilityToken`).
- Tareas manuales: 1 marcada OBSOLETA, 2 marcadas HECHO, 1 PARCIAL, 1 nueva (branch protection).
- Tabla de PRs: agregado #40; #37-#39 marcados como sin registrar (no se inventa su contenido).
